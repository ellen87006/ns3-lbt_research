  ...
  # Energy detection threshold for LAA
            for cell in Wifi  ; do
              for rules in all any ;do 
                    ...
                    ./waf --run laa-wifi-indoor --command="%s --ftpLambda=${ftpLambda} --cellConfigA=Wifi --cellConfigB=Wifi --logWifiRetries=1 --logWifiFailRetries=1 --logPhyArrivals=0 --transport=${transports} --duration=${duration} --cwUpdateRule=${cwUpdateRules}  --logHarqFeedback=1 --logTxops=1 --logCwChanges=1 --logBackoffChanges=1 --simTag=${simTag} --outputDir=${outputDir}  --RngRun=${RngRun}"
                done
              done
done
